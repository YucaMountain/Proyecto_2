module m2_DeBounce (
    input  logic clk,      // Usaremos el clk de 1kHz generado arriba
    input  logic rst_n,
    ...
    logic [4:0] count;
    logic sync_0, sync_1; // Sincronizadores para evitar metaestabilidad

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            sync_0 <= 1'b1; // Recordar que las filas tienen Pull-up (inactivas en 1)
            sync_1 <= 1'b1;
            sw_out <= 1'b1;
            count  <= '0;
        end else begin
            // Paso 1: Sincronización para evitar problemas de timing asíncrono
            sync_0 <= sw_in;
            sync_1 <= sync_0;

            // Paso 2: Verificación de estabilidad
